...
Provide honest, constructive feedback focusing on how well the prompt communicates intent, provides necessary context, and would generate useful responses.`;
    ...
    /**
     * Check if user can perform operation within word limits and calculate credits
     */
    async checkLimitsAndCredits(userId, inputWords, estimatedOutputWords) {
        ...
        const result = {
            ...
            limitExceeded: false,
            userPlan,
            limits: this.WORD_LIMITS,
            message: ''
        };
        
        // Check one-time input word limit
        if (inputWords > this.WORD_LIMITS.inputWords) {
            result.canProceed = false;
            result.limitExceeded = true;
            result.message = `Input exceeds limit of ${this.WORD_LIMITS.inputWords} words. Please reduce your input.`;
            return result;
        }
        
        // Check one-time output word limit
        if (estimatedOutputWords > this.WORD_LIMITS.outputWords) {
            result.canProceed = false;
            result.limitExceeded = true;
            result.message = `Estimated output exceeds limit of ${this.WORD_LIMITS.outputWords} words. Please reduce your request.`;
            return result;
        }
        
        // Calculate credits needed for all users
